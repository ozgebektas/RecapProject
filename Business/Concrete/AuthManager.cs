...
namespace Business.Concrete
{
    public class AuthManager : IAuthService
    {
        ...
        public IDataResult<User> Login(UserForLoginDto userForLoginDto)
        {
            ...
            if (userToCheck == null)
            {
                return new ErrorDataResult<User>(UserMessages.UserNotGetByEmail);
            }
            if (!HashingHelper.VerifyPasswordHash(userForLoginDto.Email, userToCheck.PasswordHash, userToCheck.PasswordSalt))
            {
                return new ErrorDataResult<User>(UserMessages.PasswordError);
            }
            return new SuccessDataResult<User>(userToCheck,UserMessages.SuccessLogin);//mevcut kullanıcı ile mesajı döndürdük
        }
